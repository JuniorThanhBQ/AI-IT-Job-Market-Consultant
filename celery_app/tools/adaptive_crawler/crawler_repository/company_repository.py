...
from app.utils.embeddings import DEFAULT_EMBEDDING_MODEL, generate_embedding_async
...
class CompanyRepository:
    ...
    def _update_company_fields(target: Company, source: Company) -> None:
        ...
        if source.description:
            if (
                "is an active technology employer" not in source.description
        ...
        if source.location:
            if (
                source.location.strip().lower() != "vietnam"
                or not target.location
                or target.location.strip().lower() == "vietnam"
            ):
