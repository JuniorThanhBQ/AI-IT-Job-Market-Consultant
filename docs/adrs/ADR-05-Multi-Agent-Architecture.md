...
## Decision
Select a Hierarchical Multi-Agent System (HMAS) architecture for orchestrating AI agents.
...
2. **Easier Debugging and Monitoring:** With a supervisor agent orchestrating sub-agents (e.g., crawler agent, profile parser, matching agent), it is simpler to trace execution paths, catch failure states, and audit LLM prompts.
...
4. **Predictable Output Quality:** Hierarchical control enforces structured data schemas at each stage (e.g., validating crawler results before passing them to the matching agent), yielding more reliable and accurate recommendations for the end user.

## Consequences
### Positive
* **Predictable Execution Workflows:** Tasks flow sequentially through structured steps (Scraping -> Parsing -> Matching -> Report Generation), minimizing random agent behavior.
* **Modular Agent Roles:** Each sub-agent is dedicated to a specific task, making it easy to upgrade individual agent logic (using LangGraph/LangChain) without affecting other agents.
* **Robust Error Handling:** If a sub-agent fails (e.g., a crawler fails to fetch a website), the supervisor agent can catch the error and execute a fallback strategy rather than crashing the entire pipeline.
