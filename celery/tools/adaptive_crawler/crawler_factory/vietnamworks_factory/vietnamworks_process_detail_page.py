...
async def process_detail_page(
    ...
    address_tag = soup.find(
        "a",
        href=re.compile(r"google\.com/maps\?q=", re.I),
    )
    ...
    if location == "Vietnam":
        location_candidates = [
            soup.find(
                string=re.compile(
                    r"Hưng Yên|Hà Nội|TP\. Hồ Chí Minh|Đà Nẵng|Việt Nam", re.I
                )
            ),
            soup.find(string=re.compile(r"Địa điểm|Địa chỉ|Location", re.I)),
        ]
