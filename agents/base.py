...
class BaseAgent(ABC):
    ...
    Provides:
    - Consistent logging (agent name, duration, status)
    - Error wrapping (exceptions are caught and written to state)
    - A standard ``run()`` contract for node functions
    """
    ...
    async def run(self, state: AgentState) -> dict[str, Any]:
        ...
        Override ``execute()`` — not this method.
        """
