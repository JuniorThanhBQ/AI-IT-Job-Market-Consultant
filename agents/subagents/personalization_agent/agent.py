...
class PersonalizationAgent(BaseAgent):
    ...
    async def execute(self, state: AgentState) -> dict[str, Any]:
        ...
        # ── 1. Scan Context for Skill Frequencies (Docker, Kubernetes, AWS, etc.) ──
        ...
        # ── 6. Enforce Resume Improvement trigger ──
        if score < 70:
            if (
                not parsed.get("resume_improvement")
                or parsed["resume_improvement"] == "null"
            ):
                # Ensure the LLM generated suggestions if it forgot
        ...
        # ── 7. Render Vietnamese Text for User Response ──
    ...
    def _count_skill_frequencies(rag_context: str) -> dict[str, int]:
        ...
        counts = {skill: 0 for skill in target_skills}
